...
<div class="container">
  ...
  <form method="POST" action="<?= site_url('appointments/add') ?>">
    <label>Client</label>
  ...
  <?php endforeach; ?>
    ...
    <select name="service_id" id="serviceSelect" required>
      ...
      <?php foreach ($services as $service): ?>
        <option value="<?= $service['id'] ?>" data-specialization="<?= htmlspecialchars($service['service_name']); ?>"><?= htmlspecialchars($service['service_name']); ?></option>
      <?php endforeach; ?>
    </select>

    <label>Veterinarian</label>
    <select name="vet_id" id="vetSelect" required disabled>
...
serviceSelect.addEventListener('change', function() {
  ...
  <?php foreach ($vets as $vet): ?>
  {
    ...
    if (vetSpec === serviceSpecialization) {
      newOptions += '<option value="<?= $vet['id']; ?>" data-specialization="' + vetSpec + '"><?= htmlspecialchars($vet['name']); ?> (' + vetSpec + ')</option>';
      matchCount++;
    }
  }
  <?php endforeach; ?>
...
async function checkAvailableSlots() {
  ...
  try {
    const response = await fetch('<?= site_url('appointments/getBookedSlots') ?>?date=' + selectedDate + '&vet_id=' + selectedVet);
    const data = await response.json();
    bookedSlots = data.bookedSlots || [];
    
    // Rebuild the time select with only available slots
